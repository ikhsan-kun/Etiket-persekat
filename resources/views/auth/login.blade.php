        ...
        <div class="min-h-screen flex flex-col md:flex-row">
            ...
            <div class="hidden md:flex md:w-1/2 lg:w-3/5 bg-dark-900 relative items-center justify-center overflow-hidden">
                ...
                <div class="absolute inset-0 z-0">
                    ...
                    <!-- Large Glow -->
                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-primary-600/20 rounded-full blur-[120px] z-0"></div>
                </div>
                ...
                <div class="relative z-20 w-full max-w-lg px-8 text-left">
                    ...
                    <div class="flex gap-4">
                        <a href="{{ route('home') }}" class="inline-flex items-center text-sm font-bold uppercase tracking-wider text-dark-400 hover:text-white transition-colors">
            ...
            <div class="flex-1 flex items-center justify-center p-6 sm:p-12 lg:p-16 bg-dark-950 relative z-10">
                ...
                <div class="w-full max-w-md mt-16 md:mt-0">
                    ...
                    <!-- Session Status -->
                    @if (session('status'))
                        <div class="mb-6 p-4 rounded-xl bg-success-500/10 border border-success-500/20 text-success-500 text-sm font-medium">
                            {{ session('status') }}
                        </div>
                    @endif
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-6">
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                <label for="password" class="block text-xs font-bold uppercase tracking-wider text-dark-300">Password</label>
                                @if (Route::has('password.request'))
                                    <a class="text-xs font-bold text-primary-500 hover:text-primary-400 transition-colors" href="{{ route('password.request') }}">
                                        Lupa Password?
                                    </a>
                                @endif
                            </div>
                        ...
                        <div class="flex items-center">
                            <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                                <div class="relative flex items-center justify-center w-5 h-5 mr-3">
                                    ...
                                    <div class="w-5 h-5 border-2 border-dark-600 rounded bg-dark-900 peer-checked:bg-primary-600 peer-checked:border-primary-600 transition-all"></div>
                                    <svg class="absolute w-3 h-3 text-white opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>
                                </div>
                                <span class="text-sm font-medium text-dark-400 group-hover:text-white transition-colors">Ingat Saya</span>
                            </label>
                        </div>
                        ...
                        <div class="text-center mt-8">
                            <p class="text-sm font-medium text-dark-400">
                                Belum punya akun? 
                                <a href="{{ route('register') }}" class="text-white font-bold hover:text-primary-400 transition-colors border-b-2 border-primary-500/30 hover:border-primary-500 pb-1 ml-1">Daftar disini</a>
                            </p>
                        </div>
                    </form>
                </div>
